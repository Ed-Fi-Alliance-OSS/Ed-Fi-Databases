...
namespace EdFi.Db.Deploy.DatabaseCommands
{
    public abstract class DeployDatabaseFromPathCommand : IDatabaseCommand
    {
        ...
        public DatabaseCommandResult Execute(IOptions options)
        {
            ...
            {
                var results = RunScripts(path, true, options.StandardVersion);
            ...
            foreach (string path in filePaths)
            {
                var results = RunScripts(path, false, options.StandardVersion);
                commandResults.Add(results);
            ...
            DatabaseCommandResult RunScripts(string path, bool isStructureScripts, string standardVersion)
            {
                config.ParentPath = path;
                config.ScriptPath = ScriptsPath(new ScriptPathResolver(path, options.DatabaseType, options.Engine, standardVersion: standardVersion));
